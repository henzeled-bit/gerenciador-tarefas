...
export default function PainelAdmin({ tarefas, onUpdate }) {
  ...
  const stats = useMemo(() => {
    // Filtrar tarefas válidas (remover undefined/null)
    const tarefasValidas = tarefasFiltradas.filter(t => t && t.id)
    
    const total = tarefasValidas.length
    const concluidas = tarefasValidas.filter(t => t.status === 'concluido').length
